...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let webViewConfig = WKWebViewConfiguration() // 대신해주는 애를 여러명 써줄수 있음 전에는 델리게이트는 self로 지정했음, 이거는 메시지 이름을 통해서 대신해주는 애를 구분함
...
//        userContnet.add(handler, name: "SetUser")
        ...
        let frame = CGRect(origin: view.frame.origin, size: CGSize(width: view.frame.width, height: view.frame.height/3 * 2)) // 버튼을 놓으려고 프레임을 조정한거임, 루트 뷰의 영역을 비워두고 나머지에 웹 정보를 보여줌
...
//        여기에 있는 데이터를 json으로 바꿔서 이기종간에도 이해하도록 바꿔줌, json 만들고 직렬화 해서 보냄
        let info = ["phone":"010-1111-1111","email":"a@a.com","name":"하하하","gender":"male"]
        do { // JSONSerialization는 throw로 보낼수도 있기 때문에 try-catch로 핸들링 해줘야 함???
            ...
            webView?.evaluateJavaScript("call_func3('\(encodedDate)')", completionHandler: { result, error in // evaluateJavaScript 메소드를 사용하여 웹 뷰에서 JS 함수를 호출하고, encodedDate를 인자로 전달함, 웹에 있는 call_func3()함수에서 반환한 결과가 completionHandler의 result에 담기게 됩니다.
                if let error {
                    return
                }
                guard let info = result as? [String:String] else {return} // 반환값을 받을 경우 completionHandler가 필요, 보통 json타입을 [String:String] 타입으로 바꿈
                print(info)
            })
            } catch {
            print("에러발생")
...
extension ViewController: WKScriptMessageHandler {
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "SendMessage" {
            ...
            print("SetUser")
            if let info = message.body as? [String:String] {// string 타입을 string string 딕셔너리의 옵셔널로 바꾼것, message.body의 데이터 타입은 Any로 되어 있으며, 전송한 info 객체의 형태는 기본적으로 js 객체이기 때문에 스위프트는 이 객체를 적절한 타입으로 변환하여 사용해야 함
